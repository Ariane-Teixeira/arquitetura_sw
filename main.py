...
from adapters.scikit_learn_adapter import ScikitLearnAdapter
import subprocess  # Import subprocess to run Streamlit

# Application
from application.use_cases import MLUseCases
from adapters.streamlit_adapter import StreamlitAdapter  # Import the Streamlit adapter
...
def main():
    ...
    dl_parser.add_argument("kaggle_name", help="Kaggle dataset name (e.g. iamsouravbanerjee/world-population-dataset)")
    ...
    # streamlit
    streamlit_parser = subparsers.add_parser("streamlit", help="Launch Streamlit to display profiling report")
    streamlit_parser.add_argument("csv_filename", help="CSV file to profile and display with Streamlit")
    ...
    training_adapter = ScikitLearnAdapter()
    streamlit_adapter = StreamlitAdapter()  # Instantiate the Streamlit adapter
    ...
    if args.command == "download":
        ...
        logger.info(f"Launching Streamlit to display profiling report for {args.csv_filename}...")
        subprocess.run(["streamlit", "run", "adapters/streamlit_adapter.py", "--", args.csv_filename])
